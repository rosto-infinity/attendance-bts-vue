...
<div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-6 max-w-2xl mx-auto">
  ...
  <form class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
      ...
      <input type="time" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
    </div>
    <div>
    ...
    <div>
      ...
      <input type="number" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
    </div>
    <div>
    ...
    <div class="md:col-span-2 flex justify-end">
      <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors duration-200">Ajouter</button>
    </div>
  </form>
</div>
      ...
      <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-6">
        <div class="flex flex-col lg:flex-row gap-4">
          ...
          <div class="flex items-center gap-2">
            ...
            <select v-model="filterStatus" @change="setCurrentPage(1)" class="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                <th @click="handleSort('date')" class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors duration-200">
                  <div class="flex items-center gap-1">Date <SortIcon field="date" /></div>
                </th>
                <th @click="handleSort('fullName')" class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors duration-200">
                  <div class="flex items-center gap-1">Nom et Prénom <SortIcon field="fullName" /></div>
                </th>
                <th @click="handleSort('email')" class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors duration-200">
                  <div class="flex items-center gap-1">E-mail <SortIcon field="email" /></div>
                </th>
                <th @click="handleSort('arrivalTime')" class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors duration-200">
                  <div class="flex items-center gap-1">Arrivée <SortIcon field="arrivalTime" /></div>
                </th>
                <th @click="handleSort('departureTime')" class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors duration-200">
                  <div class="flex items-center gap-1">Départ <SortIcon field="departureTime" /></div>
                </th>
                <th @click="handleSort('lateMinutes')" class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors duration-200">
                  <div class="flex items-center gap-1">Retard (min) <SortIcon field="lateMinutes" /></div>
                </th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="record in paginatedData" :key="record.id" class="hover:bg-gray-50 transition-colors duration-200">
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">{{ new Date(record.date).toLocaleDateString('fr-FR') }}</td>
                <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm font-medium text-gray-900">{{ record.fullName }}</div></td>
                ...
                <td class="px-6 py-4 whitespace-nowrap"><Badge :type="record.isAbsent ? 'danger' : 'success'">{{ record.isAbsent ? 'Oui' : 'Non' }}</Badge></td>
                <td class="px-6 py-4 whitespace-nowrap"><Badge :type="record.isLate ? 'warning' : 'success'">{{ record.isLate ? 'Oui' : 'Non' }}</Badge></td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="px-6 py-4 border-t border-gray-200">
          <div class="flex flex-col lg:flex-row items-center justify-between gap-4">
            <div class="flex items-center gap-2 text-sm text-gray-700">
              <span>Afficher</span>
              <select v-model="itemsPerPage" @change="setCurrentPage(1)" class="px-2 py-1 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <div class="flex items-center gap-2">
              <button @click="setCurrentPage(Math.max(1, currentPage - 1))" :disabled="currentPage === 1" class="p-2 rounded-lg border border-gray-300 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200">
                <ChevronLeft class="w-4 h-4" />
              </button>

              <div class="flex items-center gap-1">
                <button v-for="pageNum in pageNumbers" :key="pageNum" @click="setCurrentPage(pageNum)" :class="['w-8 h-8 rounded-lg text-sm font-medium transition-all duration-200', currentPage === pageNum ? 'bg-blue-600 text-white' : 'border border-gray-300 hover:bg-gray-50']">{{ pageNum }}</button>
              </div>

              <button @click="setCurrentPage(Math.min(totalPages, currentPage + 1))" :disabled="currentPage === totalPages" class="p-2 rounded-lg border border-gray-300 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200">
